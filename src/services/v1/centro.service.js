...
import {
  ...
  checkCentroHasSupervisoresRepository,
...
export const changeCentroStatusService = async (code, nuevoEstado) => {
  ...
  // Si se quiere desactivar el centro, verificar que no tenga supervisores asociados
  if (estadoBoolean === false) {
    const hasSupervisores = await checkCentroHasSupervisoresRepository(code);
    if (hasSupervisores) {
      const error = new Error("No se puede desactivar el centro porque tiene supervisores asociados");
      error.code = "CENTRO_HAS_SUPERVISORES";
      throw error;
    }
  }
...
/**
 * Servicio para verificar si un centro tiene supervisores asociados.
 */
export const checkCentroHasSupervisoresService = async (codigo) => {
  const hasSupervisores = await checkCentroHasSupervisoresRepository(codigo);
  if (hasSupervisores) {
    const error = new Error("El centro tiene supervisores asociados");
    error.code = "CENTRO_HAS_SUPERVISORES";
    throw error;
  }
  return hasSupervisores;
};