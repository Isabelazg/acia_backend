...
export const getUsersRepository = async ({
    ...
    if (nombre_usuario) where.nombre_usuario = { [Op.like]: `%${nombre_usuario}%` };
    ...
    const allowedSort = [
        "id", "documento", "nombres", "apellidos", "nombre_usuario", "correo", "telefono", "estado"
    ];
    ...
    // Prioridad: centro_filtro (desde middleware) > centro_id (desde query)
    const filtro_centro = centro_filtro || centro_id;
    ...
    if (filtro_centro) {
        include.push({
            ...
            through: { attributes: [] },
            where: { id: filtro_centro },
            required: true // Solo usuarios que tengan ese centro
        });
    } else {
        // Si no se envía centro_id ni centro_filtro, mostrar usuarios sin filtrar por centro
        include.push({
            association: 'centros',
            attributes: ['id', 'nombre', 'codigo'],
            through: { attributes: [] }
