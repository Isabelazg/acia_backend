...
/**
 * Verificar si un centro tiene supervisores asociados.
 */
export const checkCentroHasSupervisoresRepository = async (codigo) => {
  const count = await Centro.count({
    where: {
      codigo,
      supervisores_id: { [Op.ne]: null }
    },
  });
  return count > 0;
};
...
/**
 * Repositorio para obtener los centros de una regional por su ID.
 */
export const getCentrosByRegionalRepository = async (regionalId, estado, sortBy = "id", order = "ASC") => {
  const whereClause = { regional_id: regionalId };
  if (estado !== undefined) {
