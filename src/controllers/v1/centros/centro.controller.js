...
import { getAreasByCentroRepository } from "../../../repositories/centro.repository.js";
 ...
 */
export const getAreasByCentro = async (req, res) => {
  try {
    const centroId = req.params.id;
    const areas = await getAreasByCentroRepository(centroId);
    if (!areas) {
      return errorResponse(res, "No existe un centro con ese ID", 404, [
        {
    ...
    return successResponse(res, areas, 200);
  } catch (error) {
    return errorResponse(res, "Error al obtener las áreas del centro", 500, [
      {
        code: "GET_AREAS_BY_CENTRO_ERROR",
        detail: error.message,
      },
    ]);
  }
